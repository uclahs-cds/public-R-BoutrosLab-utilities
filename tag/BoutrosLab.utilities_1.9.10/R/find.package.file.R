# The BoutrosLab.utilities package is copyright (c) 2013 Ontario Institute for Cancer Research (OICR).
...
find.package.file <- function(package, file) {

    # Find the location of all packages
    paths.to.check <- paste(.libPaths(), package, file, sep = '/');
    file.checks <- file.exists(paths.to.check);

    # Return any directories that contain the file
    return(paths.to.check[file.checks]);
    }
